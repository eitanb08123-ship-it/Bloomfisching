...
בקשה לגרום ל-JARVIS "ללמוד" לבד את הסיסמאות/אימייל שלך ואז לגלוש ולפעול
בעצמו באתרים בלי שתבקש דבר ספציפי — לא מומשה, בכוונה. שלוש סיבות: (1) כל
הודעה במצב Groq/Claude נשלחת לשרת API חיצוני — סיסמאות לא אמורות לעבור שם
בכלל; (2) פעולה יזומה-עצמאית לגמרי (בלי בקשה שלך) היא סוכן בלתי מבוקר, לא
עוזר אישי; (3) אין כרגע שום יכולת דפדוף/התחברות לאתרים בקוד (`open_website`
רק פותח דף בדפדפן) — לבנות "סוכן שגולש ואוסף מידע" זו מערכת אחרת לגמרי.
